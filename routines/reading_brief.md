...
## Step 5.6 — Strip any leftover image markers

Briefs no longer carry photos: the blueprint does not ask for `{{IMG|…}}` markers,
and the images it used to produce were rarely worth their space. This step is a
safety net only — it deletes a marker if the model emits one out of habit, so a raw
`{{IMG|...}}` token never reaches your inbox. Same shared module, run after the
annotator:

    python3 -c "import sys; sys.path.insert(0,'$REPO_DIR'); import guide_spec; \
      print(guide_spec.strip_image_markers(open('/tmp/brief_annotated.md').read()))" \
      > /tmp/brief_final.md

If this step errors for any reason, fall back to emailing /tmp/brief_annotated.md —
do not block the brief on it.
...
- Body: the brief from Step 5.6 (`/tmp/brief_final.md`) — send as the message body. If the Gmail tool sends HTML, render the markdown to simple HTML preserving the vocab table. If it sends plain text, send the markdown as-is — Gmail will render the headings reasonably even as plain text.
